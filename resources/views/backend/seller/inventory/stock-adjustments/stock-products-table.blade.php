                    ...
                    <table class="min-w-full">
                        <thead class="bg-theme-primary/10 border-b border-border">
                            <tr class="text-left">
                                ...
                                <th class="p-4 {{ !Route::is('seller.stockAdjustments.show') ? 'w-[200px]' : 'w-[280px]' }}"
                                    data-breakpoints="xs sm">
                                    {{ translate('Product') }}
                                </th>
                                @if (!Route::is('seller.stockAdjustments.show'))
                                ...
                                <th class="p-4 {{ !Route::is('seller.stockAdjustments.show') ? '' : 'text-end' }}"
                                    data-breakpoints="xs sm">
                                    {{ translate('Qty') }}
                                </th>
                            ...
                            @if (count($stockProductVariations) > 0)
                                @foreach ($stockProductVariations as $key => $stockProductVariation)
                                    ...
                                    <tr
                                        ...
                                        <td
                                            class="px-6 py-4 whitespace-nowrap text-sm font-medium text-foreground capitalize">
                                            {{ $stockProductVariation->action }}</span>
                                        </td>
