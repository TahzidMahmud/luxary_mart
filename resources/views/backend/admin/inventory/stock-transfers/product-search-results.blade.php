@if (count($productVariations) > 0)
    @foreach ($productVariations as $key => $variation)
        @php
            ...
            $discount = variationPrice($product, $variation, false) - variationDiscountedPrice($product, $variation, false);
            $subtotal = $variation->price + $tax - $discount;
            $selectedVariationIds = $selectedVariationIds ?? [];
        @endphp

        <a class="cursor-pointer stock-product block py-4 px-6 hover:bg-theme-primary/10"
            onclick="handlePoProductClick(this)" data-variation-id="{{ $variation->id }}"
